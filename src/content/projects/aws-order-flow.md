...
body:
  ...
  decisions:
    - decision: "Split the system into multiple CDK stacks instead of one."
      ...
      why: "This let the data layer be replaced without touching the API or messaging stacks — it's what made the Aurora migration tractable."
    - decision: "Wrote the migration script idempotent and re-runnable."
      ...
      why: "This meant a partial run could be repeated safely instead of requiring the target cleaned first."
    - decision: "Used contract tests comparing API responses either side of the cutover."
      ...
      why: "Row counts only prove the data arrived, not that every field still means what callers expect — comparing full API responses on both sides catches representation differences a count can't see."
